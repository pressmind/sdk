...
**Purpose:** Downloads and processes images, creates derivatives, and can reset missing images for reprocessing. Optionally generates a verification report (storage scan with statistics).

| Argument / Option | Description |
|-------------------|-------------|
| *(none)* | Process all pending images. Verification report is **not** generated by default. |
| `unlock` | Removes the image processor process lock. |
| `reset-missing` | Runs verification and sets `download_successful=0` for incomplete Pictures, Sections and DocumentMediaObjects. Incomplete means missing derivative row, missing/empty derivative file, missing/empty WebP sidecar, or duplicate derivative rows. Can be combined with `mediaobject <ids>`. |
| `mediaobject` | Second argument: `<id>[,<id>...]`. Process only images for the given media object(s). IDs must be positive integers; invalid or partially numeric values such as `123abc` are ignored. The filter applies to cleanup, pending processing, optional report and missing-image reset, and includes Pictures, Sections and DocumentMediaObjects. |
| `--report` | Generate the verification report after processing (storage scan, derivative statistics, missing-image reset). The report includes concrete missing keys and duplicate derivative names. This is expensive on large buckets and therefore **off by default**. |
...
php cli/image_processor.php --report
php cli/image_processor.php unlock
php cli/image_processor.php reset-missing
php cli/image_processor.php reset-missing mediaobject 12345,12346
php cli/image_processor.php mediaobject 12345,12346
php cli/image_processor.php mediaobject 12345,12346 --report
```
...
> **Note:** When the image processor is spawned automatically after an import (`postImport()`), `--report` is not passed, so verification is skipped. This keeps the import pipeline fast and memory-efficient. Run `--report` manually when you need storage diagnostics.
>
> `mediaobject` IDs are sanitized. Manual `mediaobject 0,abc,123def` exits with an error. Automatic post-import spawning never passes `mediaobject 0`; invalid or empty ID lists are ignored.
