    ...
    public function map($pIdMediaObject,$pLanguage, $pVarName, $pObject)
    {
        ...
        if(is_array($pObject)) {
            foreach ($pObject as $object) {
                $check = \Pressmind\ORM\Object\MediaObject\DataType\Picture::listAll(['id' => $object->id_media_object]);
                if(count($check) > 0) {
                    $mapped_object = new \Pressmind\ORM\Object\MediaObject\DataType\Picture($check[0]->id);
                } else {
