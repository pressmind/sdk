...
class MediaObjectType extends AbstractImport implements ImportInterface
{
    ...
    public function import($drop_tables = false)
    {
        ...
        foreach ($response->result as $result) {
            $this->_log[] = $this->_getElapsedTimeAndHeap() . ' Importer::importMediaObjectTypes(' . implode(',' ,$this->_ids) . '): Starting scaffolding for ID: ' . $result->id;
            ...
            $this->_log[] = $this->_getElapsedTimeAndHeap() . ' Importer::importMediaObjectTypes(' . implode(',' ,$this->_ids) . '): Scaffolding for ID: ' . $result->id . ' finished';
        }
    }
}
