...
class Calendar extends AbstractIndex
{
  ...
  public function createCalendar($idMediaObject, $language, $origin, $agency = null)
  {
    ...
    foreach($items as $k => $item){
      $query = 'select 
                      duration, id_booking_package 
                      from (
                        SELECT state,
                               duration,
                               id_booking_package,
                               price_total,
                               ROW_NUMBER() OVER (PARTITION BY duration, id_booking_package 
    ...
    foreach($items as $item){
      foreach($item['durations'] as $duration){
        ...
        foreach($offers as $offer){
          $date_key = $offer->date_departure->format('Y-m-d');
          $current_state_priority = array_search($offer->state, CheapestPrice::STATE_ORDER_BY_PRIO);
          if (empty($date_to_cheapest_price[$date_key])) {
            $date_to_cheapest_price[$date_key] = $offer;
          } else {
            $existing_state_priority = array_search($date_to_cheapest_price[$date_key]->state, CheapestPrice::STATE_ORDER_BY_PRIO);
            if ($current_state_priority < $existing_state_priority) {
              $date_to_cheapest_price[$date_key] = $offer;
            } elseif ($current_state_priority == $existing_state_priority) {
                if ($offer->price_total < $date_to_cheapest_price[$date_key]->price_total) {
                $date_to_cheapest_price[$date_key] = $offer;
              }
            }
          }
