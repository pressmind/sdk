...
class VerificationStatsTest extends AbstractTestCase
{
    ...
    public function testNonPrefixListingDoesNotCountFilesizeErrorsAsExisting(): void
    {
        $picture = new Picture();
        $picture->id = 1;
        $picture->id_media_object = 123;
        ...
        $method = new \ReflectionMethod(VerificationStats::class, 'verifyPictures');
        ...
        $args = [
            ...
            100,
            &$derivativeSummaryMap,
            $this->createBucketWithProviderThatCannotReadSize(),
        ];
        $method->invokeArgs(null, $args);

        $this->assertSame(0, $stats['pictures']['exists']);
        $this->assertSame(1, $stats['pictures']['missing']);
        $this->assertFalse($stats['pictures']['derivatives'][0]['derivatives'][0]['exists']);
        $this->assertSame(0, $derivativeSummaryMap['teaser.jpg.pictures']['exists_count']);
        $this->assertContains('image_teaser.jpg', $stats['pictures']['missing_list'][0]['missing_keys']);
    }
    ...
    public function testStreamingVerificationKeepsSubdirectoryKeys(): void
    {
        $bucketPath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'pm_verify_stream_subdir_' . uniqid('', true);
        mkdir($bucketPath, 0755, true);
        $bucket = new Bucket(['provider' => 'filesystem', 'bucket' => $bucketPath]);
        $this->writeBucketFile($bucket, 'subdir/image_teaser.jpg', 'jpg');
        ...
        $picture->download_successful = true;
        $picture->derivatives = [
            $this->createPictureDerivative(10, 1, 'teaser', 'subdir/image_teaser.jpg'),
        ];
        Registry::getInstance()->add('db', $this->createVerificationDb([$picture]));
        $config = [
            'image_handling' => [
    ...
    {
        $bucketPath = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'pm_verify_size_error_' . uniqid('', true);
        mkdir($bucketPath, 0755, true);
        $bucket = new Bucket(['provider' => 'filesystem', 'bucket' => $bucketPath]);
