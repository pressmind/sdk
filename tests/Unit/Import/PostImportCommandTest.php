...
namespace Pressmind\Tests\Unit\Import {
    ...
    class PostImportCommandTest extends AbstractTestCase
    {
        ...
        /**
         * @dataProvider emptyPostImportIdsProvider
         */
        public function testPostImportWithoutValidIdsStartsImageProcessorWithoutMediaobjectArgument($ids): void
        {
            $this->configureImportRuntime();
            PostImportCommandExecSpy::activate([]);
            ...
            $this->assertTrue(
                $this->commandsContain($commands, '/cli/image_processor.php'),
                'Expected postImport() to start image_processor.php. Commands: ' . implode("\n", $commands)
            );
            $this->assertFalse(
                $this->commandsContain($commands, 'mediaobject 0'),
                'postImport() must not turn empty IDs into mediaobject 0. Commands: ' . implode("\n", $commands)
            );
            $this->assertFalse(
                $this->commandsContain($commands, 'mediaobject'),
                'postImport() without valid IDs must start an unfiltered image processor. Commands: ' . implode("\n", $commands)
            );
        }

        public static function emptyPostImportIdsProvider(): array
        {
            return [
                'null' => [null],
                'empty array' => [[]],
        ...
        public function testPostImportSanitizesMediaobjectIds(): void
        {
            $this->configureImportRuntime();
            PostImportCommandExecSpy::activate([]);
            ...
            ob_start();
            try {
                $import->postImport([0, '123', 'abc', '789abc', '12.9', 123, 456]);
            } finally {
                ob_end_clean();
            }
            ...
            $commands = PostImportCommandExecSpy::getCommands();
            $this->assertTrue(
                $this->commandsContain($commands, 'mediaobject 123,456'),
                'Expected sanitized mediaobject IDs without zeroes or duplicates. Commands: ' . implode("\n", $commands)
            );
            $this->assertFalse(
                $this->commandsContain($commands, 'mediaobject 0'),
                'Sanitized mediaobject IDs must not include 0. Commands: ' . implode("\n", $commands)
            );
            $this->assertFalse(
                $this->commandsContain($commands, 'mediaobject 789') || $this->commandsContain($commands, ',12,'),
                'Partially numeric IDs must not be cast into valid IDs. Commands: ' . implode("\n", $commands)
            );
        }
